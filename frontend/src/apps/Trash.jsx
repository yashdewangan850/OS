...
import { AlertTriangle, File, Folder, RefreshCw, RotateCcw, Trash2 } from "lucide-react";
import { emptyTrash, loadTrash, permanentlyDelete, restoreFromTrash } from "./trashStore";
    ...
    <div className="flex h-full min-h-0 flex-col bg-slate-50 text-slate-800">
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
        <div>
          ...
          <p className="text-xs text-slate-500">{items.length} item{items.length === 1 ? "" : "s"} in Trash</p>
        </div>
        <div className="flex gap-2">
          <button onClick={refresh} className="rounded-lg p-2 hover:bg-slate-100" title="Refresh">
            <RefreshCw size={16} />
          </button>
          <button
            onClick={() => {
              if (items.length && window.confirm("Permanently delete all items from Trash?")) emptyTrash();
            }}
        ...
        {!items.length ? (
          ...
          <div className="grid gap-2">
            {items.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
                  <div className="flex min-w-0 items-center gap-3">
                    <Icon size={20} className="shrink-0 text-blue-500" />
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">{item.name}</p>
                      <p className="text-xs text-slate-400">
                        {item.type || "file"} • Deleted {new Date(item.deletedAt).toLocaleString()}
                      </p>
                    </div>
                  </div>
                  <div className="flex shrink-0 gap-1">
                    ...
                    <button
                      onClick={() => {
                        if (window.confirm(`Permanently delete "${item.name}"?`)) permanentlyDelete(item.id);
                      }}
