...
export default function Browser() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved.current) {
        setCurrentUrl(saved.current);
        setInput(saved.current);
      }
    } catch {}
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      JSON.stringify({
        current: currentUrl,
        back: backStack.slice(-30),
        forward: forwardStack.slice(0, 30),
      }),
    );
    ...
    <div className="flex h-full min-h-0 flex-col bg-slate-100 text-slate-800">
      <div className="shrink-0 border-b border-slate-200 bg-white px-3 py-2">
        <div className="flex items-center gap-1">
          ...
          >
            <div className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 focus-within:border-blue-400 focus-within:bg-white">
        ...
        <div className="mt-2 flex gap-2 overflow-x-auto">
          {QUICK_LINKS.map(([name, url]) => (
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium hover:bg-slate-200"
            >
              <Globe size={12} />
              {name}
            </button>
      ...
      <div className="relative min-h-0 flex-1 bg-white">
        {loading && (
          <div className="absolute left-0 right-0 top-0 z-20 h-0.5 bg-slate-200">
            <div className="h-full w-1/3 animate-pulse bg-blue-500" />
          </div>
        )}
        <iframe
          key={frameKey}
          title="YashOS Browser"
          src={currentUrl}
        ...
        {blocked && (
          <div className="absolute inset-0 grid place-items-center bg-white/95 p-6 text-center">
            <div className="max-w-sm">
              ...
              />
              <h3 className="font-semibold">This site cannot be embedded</h3>
              <p className="mt-2 text-xs leading-5 text-slate-500">
                The website blocked iframe access. Open it in your normal
                browser instead.
              </p>
              <a
                href={currentUrl}
                target="_blank"
                rel="noreferrer"
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white"
              >
      ...
      <div className="flex shrink-0 items-center gap-2 border-t border-slate-200 bg-white px-3 py-1.5 text-[11px] text-slate-500">
        <Globe size={12} />
        <span className="truncate">{currentUrl}</span>
        <span className="ml-auto shrink-0">{backStack.length} history</span>
      </div>
    </div>
  );
}
